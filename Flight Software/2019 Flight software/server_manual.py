...
def threadmain():
    ...
    t.title("Western _HAB ground server")
...
def updateDisplays(message_text):
    ...
    if(identifier == "INTLZ"):
        ...
        #_HAB GPS
        t.children["_HABaltitudeBox"].configure(text=fields[4])
        #Speed @ 5
        t.children["_HABlongitudeBox"].configure(text=fields[6])
        t.children["_HABlatitudeBox"].configure(text=fields[7])
		
		#_CSA GPS
        t.children["_CSAaltitudeBox"].configure(text=fields[8])
        t.children["_CSAlongitudeBox"].configure(text=fields[9])
        t.children["_CSAlatitudeBox"].configure(text=fields[10])
