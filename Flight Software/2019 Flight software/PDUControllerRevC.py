...
class GUIThread:
	...
	def __init__(self, master):
		...
		# Refresh status when GUI starts
	...
	def run(self):
		...
		if not receiveQ.empty():
			...
			if header == "SRVCSET":  # Update service state
				nb, state = packet[1], packet[2]
				self.gui.services[int(nb)-1].setState(state)
				msg = "STATUS,"+packet[1]
				self.gui.send(msg)
			elif header == "STATUS":
				nb, state, value = packet[1], packet[2], packet[3]
...
class GUI:
	...
	def updateSetting(self, setting, editMode=False):
		...
		if editMode:
			...
			if (setting.key == "refresh_status"):
				setting.settingButton = tk.Button(
					self.settingsFrame, text="Edit", width=4, command=partial(self.updateSetting, setting, True))
			else:
				# Disable edit buttons for IP address and Port during Flight
				setting.settingButton = tk.Button(
					self.settingsFrame, text="Edit", state="disabled", width=4)
	...
	def reset(self):
		...
		answer = tk.messagebox.askokcancel(
		    "Warning", "Resetting the unit will revert the IP Address to 192.168.1.177 and the Port to 50000. Do not use during flight. Do you still want to reset the unit?")
		if answer:
			msg = "RESET"
			self.send(msg)
		else:
