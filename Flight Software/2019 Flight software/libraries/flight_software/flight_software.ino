/*
	...
	NEED TO WRITE FAN CODE. USE PINS 0/1 FOR READING?



   ***DISABLED CHECKS
   ***GPS IS SIMULATED
   ***THERMISTOR VALUES SIMULATED (CHANGE IN LIB)
   ***ACTUATOR PINS CHANGED, ALL USE ACTUATOR 1'S
   ***DOESN'T SHUT OFF ON DESCENT
   ***PRINTS OUT ALTITUDE TO SERIAL
   
   
   
   
   Potential code for restarting
   https://forum.arduino.cc/index.php?topic=404809.0
   The lib commands don't seem to reallybe different?
   CURRENTLY MAKES ADDITIONAL SD.BEGIN() CALLS IN HAB_LOGGING
   
   CHANGED HOW IT WORKS FOR ACTUATORS, NO MORE CLOSE < OPEN, JUST SET A BIG CLOSING ALT THAT IT WILL NEVER HIT


   Switching pods uses the PODS NAME.


   REENABLE THE CHECK FOR THE CONNECTION I SUPPOSE? OR IT WILL CONNECT ON ITS OWN ANYWAYS
   REENABLE BMP
...
byte sub[] = {0,0,0,0};
	...
	//Sensors and camera----------------------------------------|
		...
		//SoftwareSerial _gpsSerial(GPS_TX_PIN, GPS_RX_PIN);
        //TinyGPSPlus gpsData;
        //bool modeSet = false;
    ...
    //Ethernet, UDP, telemetry----------------------------------|
        ...
        bool gpsModeBypass = false;
        bool gpsLockBypass = false;
...
//---------------------------------------------------------------------------------------------/
    ...
    void setup() {
        ...
        //Setup objects---------------------------------------------|
            ...
            //Sets up the GPS
            //_gps = new HAB_GPS(GPS_TX_PIN, GPS_RX_PIN);
...
//---------------------------------------------------------------------------------------------/
    ...
    void loop() {
        ...
        //GPS readings----------------------------------------------|
            ...
            //SIMULATED GPS ALTITUDE
            /*if(millis()/1000 < 9000)
                _GPSreadings.altitude = (millis()/1000)*5;
            else
                _GPSreadings.altitude = 45000 - (millis()/1000 - 9000)*5;*/
            
			/*if(millis()/1000 < 150)
                _GPSreadings.altitude = (millis()/1000)*300;
            else
                _GPSreadings.altitude = 45000 - (millis()/1000 - 150)*300;*/  

            if(millis()/1000 < 1000)
                _GPSreadings.altitude = (millis()/1000)*45;
            else
                _GPSreadings.altitude = 45000 - (millis()/1000 - 1000)*45;                        
            ...
            if(pktSize){
                ...
                if(strcmp(msgPtr, PRISM_NAME) == 0){
                    ...
                    if(strcmp(msgPtr, GPS_NAME) == 0 && CSA_GPS_enabled){
                        //Latitude
                        msgPtr = strtok(NULL, FIELD_DELIMITER);             
                        _GPSreadings.latitude = atof(msgPtr);

                        //Longitude
                        msgPtr = strtok(NULL, FIELD_DELIMITER);
                        _GPSreadings.longitude = atof(msgPtr);
        ...
        void handleCommand(char* command){
            ...
            //Execute the command---------------------------------------|
                //Bypasses--------------------------------------------------|
                    ...
                    else if(!strcmp(firstArg, "GPS_MODE_BYPASS")){ gpsModeBypass = true; }
                    else if(!strcmp(firstArg, "GPS_LOCK_BYPASS")){ gpsLockBypass = true; }
                ...
                //Actuators-------------------------------------------------|
                        ...
                        if(strcmp(secondArg, "") != 0 && strcmp(thirdArg, "") != 0 && getPodIndex(secondArg) != -1 && atof(thirdArg) >= 0){ 
            ...
            //Send result message---------------------------------------|
                ...
                sendGSmessage(validCommand ? "Command executed!" : "Invalid command!");      
                   ...
                   Serial.print("Cur act : " );
                   Serial.println(_actArray[activeIndex].getName());
        }
        ...
        void checkStartupConditions(){
            ...
            //BME check-------------------------------------------------|
                ...
                if(BMPstatus = _bme.begin()){
                    ...
                    HAB_Logging::printLogln("CHECK BME");              
                    sendGSmessage("CHECK BME");
                }      

            //----------------------------------------------------------\
            //GPS check-------------------------------------------------|
                while(!_gps->isModeSet() && !gpsModeBypass){  }
                    HAB_Logging::printLogln("GPS MODE OKAY");              
                    sendGSmessage("GPS MODE OKAY");
     
                //while(!(_gps->getLockStatus()) && !gpsLockBypass){ recievePacketsUDP(); }           
